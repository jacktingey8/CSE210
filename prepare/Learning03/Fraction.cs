class Fraction()
{
    private int _top;
    private int _bottom;

    public void SetNumbers(){_top=1; _bottom=1;}
    public void SetTop(int top){_top=top;}
    public void SetBottom(int bottom){_bottom = bottom;}

    public void SetBoth(int top, int bottom){_top=top;_bottom=bottom;}

    public int GetTop(){return _top;}

     public int GetBottom(){return _bottom;}

    public string GetFractionString(){return $"{_top}/{_bottom}";}

    public double GetFractionDecimal(){return (double)_top/(double)_bottom;}

};
